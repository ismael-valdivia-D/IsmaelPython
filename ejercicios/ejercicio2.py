...
while op!=4:
    ...
    if op == 1: 
        ...
        menu1=int(input("Ingrese la opción que desea realizar: "))
        if menu1!= 2:
            try:
                if menu1 == 1:
                    sum= int(input("Ingrese el monto a pagar: "))
                    deuda = deuda + sum
                    break
            except ValueError:
                print("ingrese un numero valido(mayor a 0)")
                suma = int(input("Ingrese el monto a pagar: "))
                deuda = deuda + sum
                break
    elif op == 2:
        comp=int(input("Ingrese el monto de la compra: "))
        if comp<deuda:
            print("Compra realizada con exito")
        else:
            print("Compra no realizada, monto excede el cupo disponible")
    elif op == 3:
        print(f"El cupo disponible es de: {deuda}")
    elif op == 4:
